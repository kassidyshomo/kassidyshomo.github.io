...
#include <tuple>
#include <vector>
#include <regex>  // added to allow pattern matching for detecting SQL injection attempts
...
bool initialize_database(sqlite3* db)
{
  ...
  sql = "INSERT INTO USERS (ID, NAME, PASSWORD)" \
    "VALUES (1, 'Kassidy', 'Shomo');" \
    "INSERT INTO USERS (ID, NAME, PASSWORD)" \
    "VALUES (2, 'Vincent', 'Tevnan');" \
    "INSERT INTO USERS (ID, NAME, PASSWORD)" \
    "VALUES (3, 'Karissa', 'Shomo');" \
    "INSERT INTO USERS (ID, NAME, PASSWORD)" \
    "VALUES (4, 'Lori', 'Silbaugh');";
...
// checks for OR value=value style SQL injection patterns
bool contains_sql_injection(const std::string& sql)
{
    // this regex matches patterns like: OR 1=1, OR 'a'='a', OR "hi"="hi"
    std::regex injection_pattern(R"(\bor\b\s+(['"]?\w+['"]?)\s*=\s*\1)", std::regex_constants::icase);
    return std::regex_search(sql, injection_pattern);
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // detect and block SQL injection before executing the query
  if (contains_sql_injection(sql))
  {
      std::cout << "SQL injection attempt detected and blocked." << std::endl;
      std::cout << "Query was not executed." << std::endl;
      return false;
  }
