...
bool add_numbers(T const& start, T const& increment, unsigned long int const& steps, T& out_result)
{
    ...
    {
        // prevent overflow by checking the maximum allowed value before adding
        if (increment > 0 && out_result > std::numeric_limits<T>::max() - increment)
        {
            return false; // overflow detected: return false to notify the caller
        }

        out_result += increment;

        // extra protection for floating point types
        if constexpr (std::is_floating_point<T>::value)
        {
            if (!std::isfinite(out_result))
            {
                return false;  // floating point overflow detected: notify caller
            }
        }
    }

    return true; // addition completed safely
}
...
bool subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps, T& out_result)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // prevent underflow by checking the minimum allowed value before subtracting
        if (decrement > 0 && out_result < std::numeric_limits<T>::min() + decrement)
        {
            return false; // underflow detected: return false to notify the caller
        }

        out_result -= decrement;
        ...
        if constexpr (std::is_floating_point<T>::value)
        {
            if (!std::isfinite(out_result))
            {
                return false;  // floating point underflow detected: notify caller
            }
        }
    }

    return true; // subtraction completed safely
}
...
void test_overflow()
{
    ...
    T result = 0;  // initialize to zero
    bool success = false;

    // safe addition
    success = add_numbers<T>(start, increment, steps, result);
    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
    // use the success boolean to notify the user if overflow occurred
    if (success)
...
void test_underflow()
{
    ...
    // safe subtraction
    success = subtract_numbers<T>(start, decrement, steps, result);
    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    // use the success boolean to notify the user if underflow occurred
    if (success)
        std::cout << +result << " (Success)" << std::endl;
    else
        std::cout << "Underflow Detected!" << std::endl;

    // subtraction that causes underflow
